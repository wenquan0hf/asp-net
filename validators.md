...
|---------|:------------:|
|类型（Type）|它定义了数据类型。|
|比较值（ControlT oCompare）|它指定了输入控制中需要比较的值|
|被比较值（ValueT oCompare）|它指定了输入控制中不变的值|
|运算符（Operator）|它指定了比较的运算符，可用的值包括：相等、不等、大于等于、小于、小于等于、数据类型检查|  
这种控件的基本语法如下：  
```
...
客户端验证通过 ClientValidationFunction 来适当的完成。客户端验证例程应该用浏览器吗、能够识别的脚本语言来编写，例如JavaScript 或者 VBScript。  
